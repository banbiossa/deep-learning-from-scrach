...
class Trainer:
        ...
        def train_step(self):
                ...
                if self.current_iter % self.iter_per_epoch == 0:
                        ...
                        train_acc = self.network.accuracy(x_train_sample,
                                                          t_train_sample)
                        test_acc = self.network.accuracy(x_test_sample,
                                                         t_test_sample)
                        self.train_acc_list.append(train_acc)
                        self.test_acc_list.append(test_acc)

                        logger.debug(f"==============================")
                        logger.debug(f"epoch: {self.current_epoch}")
                        logger.debug(f"train acc: {train_acc}")
                        logger.debug(f"test acc: {test_acc}")
                self.current_iter += 1
        ...
        def train(self):
                ...
                logger.debug(f"==============================")
                logger.debug(f"Final Test Accuracy")
                logger.debug(f"test acc: {test_acc}")
